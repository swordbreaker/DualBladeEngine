...
public struct CircleCollider : ICollider
{
    ...
    private readonly bool HitTest(CircleCollider collider, out CollisionInfo info)
    {
        ...
        var direction = otherCenter - thisCenter;
        var normal = (direction.LengthSquared() > 0)
            ? Vector2.Normalize(otherCenter - this.Center)
            : direction;
    ...
    private readonly bool HitTest(RectangleCollider collider, out CollisionInfo info)
    {
        ...
        else
        {
            ...
            var edgeDistX = MathF.Min(MathF.Abs(thisCenter.X - rectBounds.X),
                MathF.Abs(thisCenter.X - (rectBounds.X + rectBounds.Width)));
            var edgeDistY = MathF.Min(MathF.Abs(thisCenter.Y - rectBounds.Y),
                MathF.Abs(thisCenter.Y - (rectBounds.Y + rectBounds.Height)));
